...
public class HttpClient {
    ...
    private void printPostRequest(HttpPost post) {
        ...
        try {
            ...
            content.read(bytes);
            System.out.println("\tcontent : " + URLDecoder.decode(new String(bytes) , "utf-8"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void printGetRequest(HttpGet get) {
        Header[] headers = get.getAllHeaders();
        System.out.println("request uri : " + get.getURI());
        System.out.println("Headers:");
        for (Header header : headers) {
            System.out.println("\t" + header.getName() + " : " + header.getValue());
        }
